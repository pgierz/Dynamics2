...
#Shallow water 2D,cyclic boundary conditions 


ni<-100  #number of gridcells in one direction
nt<-500 #number of timesteps


#The physical constants
g<-10 #low gravity, 0.1 m/s^2
...
#Inital condition: One smooth blob (sin)
#h[50:90,50:90]<-sin(0:40/2*pi/20)%*%t(sin(0:40/2*pi/20))
h[50:90,50:90]<-0.5



#1st step euler forward
u.new[ia.0,ia.0]<-u[ia.0,ia.0]-g*dt/2/dx*(h[ia.p1,ia.0]-h[ia.m1,ia.0]) +dt*f0*v

v.new[ia.0,ia.0]<-v[ia.0,ia.0]-g*dt/2/dy*(h[ia.0,ia.p1]-h[ia.0,ia.m1]) -dt*f0*u
...
for (n in 2:(nt-1))
{
	...
	u.new[ia.0,ia.0]<-u.old[ia.0,ia.0]-g*dt/dx*(h[ia.p1,ia.0]-h[ia.m1,ia.0])+dt*f0*v
	v.new[ia.0,ia.0]<-v.old[ia.0,ia.0]-g*dt/dy*(h[ia.0,ia.p1]-h[ia.0,ia.m1])-dt*f0*u
	...
	if ((n %% 10) == 0) {
		#persp(h/3, theta = 110, phi = 40, scale = FALSE, ltheta = -120, shade = 0.6, border = NA, box = FALSE,zlim=c(-0.3,0.3)) 
	 	#quiver(1:20,1:20,u[(1:20)*5,(1:20)*5],v[(1:20)*5,(1:20)*5],scale=200,maxspeed=1)
                filled.contour(h,zlim = c(-100,100))
	}
