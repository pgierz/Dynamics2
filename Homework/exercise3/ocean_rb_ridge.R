...
save_T_data = 0;	#save data of temperature field (1=yes, 0=no)
save_usq_data = 0;	#save data of u^2 field (1=yes, 0=no)
save_ux_data = 1;	#save data of ux field (1=yes, 0=no)
save_uy_data = 0;	#save data of uy field (1=yes, 0=no)
save_rho_data = 0;	#save data of rho field (1=yes, 0=no)
save_vorticity_data = 0;#save data of vorticity field (1=yes, 0=no)
###############################################################################


TCold  = 0;		#dimensionless Temperature: Cooling at the top wall
THot   = 1;		#dimensionless Temperature: Heating at the bottom wall
...
# Initialize T with average temperature
T = array(T0, c(lx, ly));

#Set small trigger to break symmetry
T[lx/2+1,1] = T[lx/2+1,1] + T0/10;
...
bottomTopo = array(0, c(lx));
for (x in 2:lx-1) {
	bottomTopo[x] =  4./5. * ly *exp( - 1/100* (x-6./7. *lx)^2 );
}
...
for (time_step in 1:max_t){
  ...
  for (i in idxRangeFluid){
    ...
    force = 3* w_fl[i]* rho * (T-T0)*
            (cx_fl[i] * g[1] + cy_fl[i] * g[2])/(THot-TCold);
  ...
  #Boundary Conditions at ridge
  for (i in 1:lx){
   for (j in 1:ly){
    if (j<bottomTopo[i]) {
     for(k in idxRangeFluid) {
      flOut[k,i,j]=flIn[opp_fl[k],i,j];
     }
     for(k in idxRangeTemp) {
      TOut[k,i,j]=TIn[opp_fl[k],i,j];
     }
    }
  }
 }

  # "Bounce Back" Boundary Conditions for Fluid (="no-slip walls")
  for (i in idxRangeFluid){
   
   #flOut[i,,1] = flIn[opp_fl[i],,1]; # bottom
    flOut[i,1,] = flIn[opp_fl[i],1,]; # left
    flOut[i,lx,] = flIn[opp_fl[i],lx,]; # right
   #flOut[i,,ly] = flIn[opp_fl[i],,ly]; # top
  }
  # "free-slip" @ top
  flOut[2,,ly] = flIn[4,,ly];
  flOut[3,,ly] = flIn[5,,ly];
  flOut[4,,ly] = flIn[2,,ly];
  flOut[5,,ly] = flIn[3,,ly];
  flOut[6,,ly] = flIn[9,,ly];
  flOut[7,,ly] = flIn[8,,ly];
  flOut[8,,ly] = flIn[7,,ly];
  flOut[9,,ly] = flIn[6,,ly];

  # Impose time-independent (but spatially-varying) temperature-profile on top
  TOut[5,2:lx-1,ly] = tempTop[2:lx-1] - TIn[1,2:lx-1,ly] - TIn[2,2:lx-1,ly] -
        TIn[3,2:lx-1,ly] - TIn[4,2:lx-1,ly];

  # "Bounce Back" Boundary Conditions for Temperature (="adiabatic walls")
  for (i in idxRangeTemp) {
    TOut[i,,1] = TIn[opp_T[i],,1]; # bottom
    TOut[i,1,] = TIn[opp_T[i],1,]; # left
    TOut[i,lx,] = TIn[opp_T[i],lx,]; # right
  }
  ...
  #TIn[5,,ly] = TCold - TIn[1,,ly] - TIn[2,,ly] - TIn[3,,ly] - TIn[4,,ly];	#Top Wall
  #TIn[3,,1] = THot - TIn[1,,1] - TIn[2,,1] - TIn[4,,1] - TIn[5,,1];	#Bottom Wall
